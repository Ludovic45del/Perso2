...
@dataclass
class FsecBean(FsecBaseBean):
    assemblyStep: list[FsecAssemblyStepBean] = None
    metrologyStep: list[FsecMetrologyStepBean] = None
    picturesStep: list[FsecPicturesStepBean] = None
    ...
    def _apply_to_steps(
        self, steps: list[any], items: list[any], apply_to_step: callable
    ):
        if not steps:
            return
