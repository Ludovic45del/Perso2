...
class GasLowPressureWithPermeationWorkflowEntity(models.Model):

    class Meta:
        app_label = "cible"
        db_table = "GAS_LOW_PRESSURE_WITH_PERMEATION_WORKFLOW"
        verbose_name_plural = "gas_low_pressure_with_permeation_workflows"
    ...
    SHOT_STATE = "shot"
    CANCELLED_STATE = "HS"
    AIRTIGHTNESS_TEST_STATE = "airtightness_test"
    PERMEATION_STATE = "permeation"
    DEPRESSURIZATION_STATE = "depressurization"
    GAS_FILLING_LOW_PRESSURE_STATE = "gas_filling_low_pressure"
    REPRESSURIZATION_STATE = "repressurization"
    ...
    STATE_CHOICES = [
        ...
        (AIRTIGHTNESS_TEST_STATE, "Test étanchéité BP"),
        (PICTURE_STATE, "Photos"),
        (PERMEATION_STATE, "Perméation"),
        (GAS_FILLING_LOW_PRESSURE_STATE, "Remplissage BP"),
        (DEPRESSURIZATION_STATE, "Dépressurisation"),
        (GAS_FILLING_LOW_PRESSURE_STATE, "Remplissage BP"),
        (REPRESSURIZATION_STATE, "Re-pressurisation"),
    ...
    def _check_transition(self, new_state):
        valid_transitions = {
            ...
            self.PICTURE_STATE: [
                self.DEPRESSURIZATION_STATE,
                self.ASSEMBLY_STATE,
                self.CANCELLED_STATE,
                self.METROLOGY_STATE,
            ],
            self.DEPRESSURIZATION_STATE: [
                self.GAS_FILLING_LOW_PRESSURE_STATE,
                self.ASSEMBLY_STATE,
                self.CANCELLED_STATE,
                self.METROLOGY_STATE,
            ],
            self.GAS_FILLING_LOW_PRESSURE_STATE: [
                self.REPRESSURIZATION_STATE,
                self.ASSEMBLY_STATE,
                self.CANCELLED_STATE,
                self.METROLOGY_STATE,
            ],
    ...
    def gas_filling_lp(self):
        self.set_state(self.GAS_FILLING_LOW_PRESSURE_STATE)

    def airthightness_test(self):
        self.set_state(self.AIRTIGHTNESS_TEST_STATE)

    def depressurization(self):
        self.set_state(self.DEPRESSURIZATION_STATE)

    def repressurization(self):
        self.set_state(self.REPRESSURIZATION_STATE)

    def permeation(self):
        self.set_state(self.PERMEATION_STATE)
