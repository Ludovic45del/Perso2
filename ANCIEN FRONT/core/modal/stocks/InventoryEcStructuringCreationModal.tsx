...
import {createInventoryEcStructuring} from "../../../services/stock/inventory.service.ts";
import {useState} from "react";
import InventoryEcStructuringModel from "../../domain/stock/InventoryEcStructuringModel.ts";

export interface ModalProps {
    isOpen?: boolean,
    handleClose: (reason?: ('saveButton' | 'cancelButton' | 'quitButton')) => void,
    handleUpdate?: () => void,
    updateList?: (list: InventoryEcStructuringModel[]) => void
}


export default function InventoryEcStructuringCreationModal({isOpen = true, handleClose, updateList}: ModalProps) {
    ...
    const sendData = () => {
        createInventoryEcStructuring(methods.getValues()).then((response: InventoryEcStructuringModel[]) => {
    ...
    return <Modal open={isOpen} onClose={() => handleClose()}>
        <Box sx={{...styleModalBox, width: 800}}>
            ...
            <p>Création d'une ligne dans les stocks (Structuration EC)</p>
